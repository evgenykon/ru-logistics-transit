...
onMounted(async () => {
  try {
    ...
    total.value = data.filter((t: any) => t.type === 'expense' && new Date(t.date) >= start).reduce((s: number, t: any) => s + t.amount, 0)
...
<template>
  <div class="stat-card">
    ...
    <span class="stat-value red">{{ total !== null ? total.toLocaleString('ru-RU') + ' ₽' : '—' }}</span>
  </div>
</template>
