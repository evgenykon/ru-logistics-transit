  ...
  try {
    ...
    const order = await prisma.order.create({
      data: {
        ...
        loadingTimeOfDay: data.loadingTimeOfDay || null,
        useLocalTime: data.useLocalTime || false,
    ...
    if (data.orderWarehouses?.length) {
      await prisma.orderWarehouse.createMany({
        data: data.orderWarehouses.map((w: any) => ({
          ...
          type: w.type || 'origin',
          loadingDate: w.loadingDate ? new Date(w.loadingDate) : null,
          loadingTime: w.loadingTime || null,
          unloadingDate: w.unloadingDate ? new Date(w.unloadingDate) : null,
          unloadingTime: w.unloadingTime || null,
        })),
      })
    }
